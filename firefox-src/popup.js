...
function maybeRequestMore(visibleCount) {
  ...
  browser.runtime.sendMessage({ action: 'requestMoreNotifications' }).catch(() => {});
}
  ...
  for (const n of filtered) {
    ...
    function markSeenAndMaybeHide() {
      if (!seenHrefs.has(href)) {
        ...
        browser.runtime.sendMessage({ action: 'markHrefSeen', href });
      }
...
async function load() {
  const [notifResponse, whitelistResponse, blacklistResponse, seenResponse, stored] = await Promise.all([
    browser.runtime.sendMessage({ action: 'getNotifications' }),
    browser.runtime.sendMessage({ action: 'getWhitelist' }),
    browser.runtime.sendMessage({ action: 'getBlacklist' }),
    browser.runtime.sendMessage({ action: 'getSeenHrefs' }),
    browser.storage.local.get(['useFilter', 'useBlacklist', 'hideClickedNotifications'])
  ]);
  ...
  blacklistToggle.addEventListener('change', () => {
    ...
    blacklistLabel.textContent = useBlacklist ? 'Blacklist on' : 'Blacklist off';
    browser.storage.local.set({ useBlacklist });
    rerender();
  });
  ...
  browser.storage.onChanged.addListener((changes, area) => {
    ...
    if (area === 'local' && changes.notifications) {
    ...
    if (area === 'local' && changes.seenHrefs) {
